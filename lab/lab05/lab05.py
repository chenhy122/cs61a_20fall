...
def closer_city(lat, lon, city_a, city_b):
    ...
    disa = distance(city_c, city_a)
    disb = distance(city_c, city_b)
    if disa < disb:
...
def sprout_leaves(t, leaves):
    ...
    for branch in branches(t):
        if is_leaf(branch):
            branch_new.append(tree(label(branch), [tree(x) for x in leaves]))
        else:
            branch_new.append(tree(label(branch),branches(sprout_leaves(branch, leaves))))
    return tree(label(t), branch_new)
...
def add_trees(t1, t2):
    ...
    if len_t1 == len_t2:
        return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in list(zip(branches(t1), branches(t2)))])
    elif len_t1 < len_t2:
        return add_trees(tree(label(t1), branches(t1) + [tree(0) for branch in range(len_t2 - len_t1)]), t2)
    else:
