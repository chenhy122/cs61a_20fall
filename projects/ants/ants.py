...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        n = 0
        while True:
            if n_place.name != beehive.name:
                if len(n_place.bees) != 0 and self.min_range <= n <= self.max_range:
                    return rANTdom_else_none(n_place.bees)
                else:
                    n_place = n_place.entrance
            else:
                return None
            n +=1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in self.place.bees[:]:
            ...
            if self.armor <= amount and bee.place != None:
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.counter == 0:
            self.throw_at(self.nearest_bee(gamestate.beehive))
            n_place = self.place.exit
            while n_place != None:
                if n_place.ant != None and n_place.ant.buffed == False:
                    n_place.ant.damage *= 2
                    n_place.ant.buffed = True
                n_place = n_place.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.armor <= 0 and self.counter == 0:
            bees_win()
        # END Problem EC
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if not (self.contained_ant or isinstance(other, ContainerAnt)):
            return True
        else:
            return False
...
def apply_status(status, bee, length):
    ...
    "*** YOUR CODE HERE ***"
    o_action = bee.action
    n_action = status(bee.action, bee)
    
    def action(gamestate):
        nonlocal length
        if length == 0:
            if bee.direction == True:
                bee.nomore_scarred = True
                bee.direction = False
            
            return o_action(gamestate)
        else:
            length -= 1
            return n_action(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while n_place != beehive:
            for bee in n_place.bees[:]:
                i_and_d[bee] = n
            if n_place.ant and n_place.ant is not self:
                i_and_d[n_place.ant] = n
            n_place = n_place.entrance
            n += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        new_damage = self.damage - distance*0.2 - self.insects_shot*0.05
        if new_damage < 0:
            return 0
        return new_damage
        # END Problem Optional 5
