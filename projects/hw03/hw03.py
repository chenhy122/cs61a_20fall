...
def balanced(m):
    ...
    left_dis, right_dis = total_weight(left_end)*length(left_arm), total_weight(right_end)*length(right_arm)
...
def mul_interval(x, y):
    ...
    value in y."""
    lower_x, lower_y = lower_bound(x), lower_bound(y)
    upper_x, upper_y = upper_bound(x), upper_bound(y)
    p1 = lower_x * lower_y
    p2 = lower_x * upper_y
    p3 = upper_x * lower_y
    p4 = upper_x * upper_y
...
def multiple_references_explanation():
    return """The Multiple References Problem: 
a formula to compute with intervals using Alyssa's system 
will produce tighter error bounds if it can be written in 
such a form that no variable that represents an uncertain number is repeated."""
...
def quadratic(x, a, b, c):
    ...
    if point >= lower and point <= upper:
        return interval(min(f(point), f(lower), f(upper)), max(f(point), f(lower), f(upper)))
    else:
        return interval(min(f(lower), f(upper)), max(f(lower), f(upper)))
